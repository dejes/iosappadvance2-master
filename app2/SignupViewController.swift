    ...
    @IBAction func RegisterBtn(_ sender: Any) {
        if EmailLabel.text?.isEmpty == true	|| PasswordLabel.text?.isEmpty == true || nickNameLabel.text?.isEmpty == true {
           ...
           present(controller, animated: true, completion: nil)
            ...
            let RegisterUserData = RegisterUser(profile:Profile(email: EmailLabel.text!, login: EmailLabel.text!, nickName: nickNameLabel.text!, gender: GenderSegment.titleForSegment(at: GenderSegment.selectedSegmentIndex)!), credentials: Credentials(password: Password(value: "Abcd1234")))
